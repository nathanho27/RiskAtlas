...
# trains random forest with expanded stock-specific risk features
...
MODEL_PATH = "models/random_forest_risk_model_v2.joblib"
...
def chronological_split(df):
    """Split data using fixed dates for fair baseline comparison."""
...
def save_predictions(test_df, y_prob, y_pred):
    ...
    pred_df = pd.DataFrame(
        {
            ...
            "actual_risk_event": (
                test_df["risk_event"].astype(int).values
            ),
            "risk_score": y_prob,
            "risk_pred": y_pred,
    ...
    print("\nV2 random forest predictions saved")
