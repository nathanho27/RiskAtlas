...
-- builds the experimental context-aware v3 modeling dataset
-- adds market, breadth, sector, beta, and correlation features
-- does not modify the current production model or dashboard tables

DROP TABLE IF EXISTS model_dataset_v3;

CREATE TABLE model_dataset_v3 AS
...
-- keep rows with known future outcomes
...
ALTER TABLE model_dataset_v3
ADD PRIMARY KEY(date,ticker);

-- improve performance when loading and splitting data by date
CREATE INDEX idx_model_dataset_v3_date
ON model_dataset_v3(date);

-- improve performance for ticker-level analysis
CREATE INDEX idx_model_dataset_v3_ticker_date
ON model_dataset_v3(ticker,date);

-- improve performance when checking label balance
CREATE INDEX idx_model_dataset_v3_risk_event
ON model_dataset_v3(risk_event);

-- improve performance for sector-level analysis
CREATE INDEX idx_model_dataset_v3_sector_date
ON model_dataset_v3(sector,date);

ANALYZE model_dataset_v3;